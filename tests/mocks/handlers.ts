...
export const handlers = [
  ...
  http.patch(`${BASE}/alerts/:id`, async ({ request, params }) => {
    ...
    return HttpResponse.json({ data: { id: params.id, type: 'alerts', attributes: { ...fixtures.alerts.single.data.attributes, ...attrs } } });
  }),
  http.post(`${BASE}/alerts`, async ({ request }) => {
    const body = await request.json() as Record<string, unknown>;
    const attrs = (body?.data as Record<string, unknown>)?.attributes as Record<string, unknown>;
    return HttpResponse.json({ data: { id: 'a-new', type: 'alerts', attributes: { status: 'triggered', ...attrs } } }, { status: 201 });
  }),
  ...
  http.post(`${BASE}/incidents`, async ({ request }) => {
    const body = await request.json() as Record<string, unknown>;
    const attrs = (body?.data as Record<string, unknown>)?.attributes as Record<string, unknown>;
    return HttpResponse.json({ data: { id: 'i-new', type: 'incidents', attributes: { status: 'started', ...attrs } } }, { status: 201 });
  }),
  http.patch(`${BASE}/incidents/:id`, async ({ request, params }) => {
    const body = await request.json() as Record<string, unknown>;
    const attrs = (body?.data as Record<string, unknown>)?.attributes as Record<string, unknown>;
    return HttpResponse.json({ data: { id: params.id, type: 'incidents', attributes: { ...fixtures.incidents.single.data.attributes, ...attrs } } });
  }),
  ...
  http.post(`${BASE}/teams`, async ({ request }) => {
    const body = await request.json() as Record<string, unknown>;
    const attrs = (body?.data as Record<string, unknown>)?.attributes as Record<string, unknown>;
    return HttpResponse.json({ data: { id: 't-new', type: 'teams', attributes: attrs } }, { status: 201 });
  }),
  http.put(`${BASE}/teams/:id`, async ({ request, params }) => {
    const body = await request.json() as Record<string, unknown>;
    const attrs = (body?.data as Record<string, unknown>)?.attributes as Record<string, unknown>;
    return HttpResponse.json({ data: { id: params.id, type: 'teams', attributes: attrs } });
  }),
  http.delete(`${BASE}/teams/:id`, () => new HttpResponse(null, { status: 204 })),

  http.get(`${BASE}/on_call_schedules`, () => HttpResponse.json({ data: [], meta: { total_count: 0 } })),
